function  [parameters]= TrainClassifierX(train_data, label)

[X,Y,mean_X,std_X] = PreProcessing(train_data,label);
X_size = size(X,2);
...
[Loss,accuracy_training,W,B] = training(X,Y,param,W,B,'adam',Ad);
...
parameters.std_X = std_X;
    ...
    function [X,Y,mean_X,std_X] = PreProcessing(train_data,label)
        ...
        numDatapnts = size(label_,1); %Total number of samples used for training/validation/testing
    ...
    function [Loss,accuracy_training,W,B] = training(X,Y,param,W,B,update,Ad)
